...
- **Auditoría de precios y renovación de vigencia (18-09-2026):** se contrastaron en vivo los precios de mercado en EE.UU. (Amazon, Newegg y B&H) para las 4 categorías volátiles (19 CPUs, 38 placas base, 30 kits de RAM DDR5 y 66 GPUs).
  ...
  - *Blindaje y preservación de fotos:* Se creó `mapping_fotos.json` (329 productos mapeados a sus 259 fotos verificadas en `public/images/catalogo/`) y se adaptó `exportar_web.py` para preservar siempre las fotos originales aunque las fuentes raw estén archivadas en JORGE, impidiendo que caigan en los iconos genéricos de `/images/components/`.
- **Configurador con enlaces híbridos a tiendas y visualización total de MPN (18-09-2026):** se mapearon los 329 componentes del catálogo del configurador (`src/data/configuradorStoreLinks.json`) y se unificó la lógica en `src/lib/presupuestoTiendas.ts`.
  - *283 productos de EE.UU.:* enlace directo a la tienda verificada (Amazon, Newegg, Best Buy, B&H) + botón de respaldo que busca el MPN exacto en la tienda alternativa (`MPN · Store`).
  ...
  - *Cobertura completa de modelos y variantes:* se eliminó la restricción que limitaba los botones a modelos de versión única. Ahora **todos los modelos compatibles** (sean multi-versión como RTX 5070 o de versión única), la cabecera del paso elegido, las versiones expandidas y el desglose del resumen lateral exhiben tanto el enlace directo como el botón de búsqueda por MPN (`MPN · Store`), además de mostrar el texto explícito del MPN en sus especificaciones y badges.
- **Presupuestos con enlaces híbridos a tiendas y MPN (18-09-2026):** los 102 componentes únicos de las 44 builds cuentan con enlaces a tiendas de EE.UU. (`src/data/presupuestoStoreLinks.json`). Cada tarjeta muestra el precio de referencia, la tienda principal verificada con botón `ExternalLink`, el MPN en sus especificaciones y el botón de respaldo por MPN exacto en la tienda alternativa.
...
- **Configuración centralizada del frontend y guía de edición rápida (18-09-2026):** se estructuró la capa `src/config/` (`siteConfig.ts`, `aboutConfig.ts`, `contactConfig.ts`) conectada a `SiteHeader`, `SiteFooter`, `SobreVista`, `ContactoVista`, `PrivacidadVista` y `TerminosVista`. Ahora es posible modificar enlaces de redes sociales, correo de soporte, menú superior, barra de anuncios superior (top banner), estadísticas y textos clave desde archivos sencillos y comentados sin tocar componentes complejos. Se redactó la guía didáctica en la raíz: `GUIA-MODIFICAR-FRONTEND.md`.
